...
export function ResearchRunPanel({ runId, onOpenReport }: { runId: string; onOpenReport: (contentId: string) => void }) {
  ...
  return <section className="research-run-panel" aria-live="polite">
    <header><span><Icon name="activity" size={16} /><strong>官方研究运行</strong></span><em className={`status-pill ${(run?.status ?? 'running').toLowerCase()}`}>{run?.status ?? 'CONNECTING'}</em></header>
    ...
    {run?.status === 'COMPLETED' && run.resultContentId && <button className="open-report-button" onClick={() => onOpenReport(run.resultContentId!)}><span><Icon name="file" size={17} /><strong>打开专业研究报告</strong><small>已核验 · 可追溯到官方 Evidence</small></span><Icon name="arrow-right" size={17} /></button>}
  </section>
}
