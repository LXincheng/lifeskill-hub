...
- “All-in-One”指用户在对话、学习、动态三个入口内完成闭环，不代表增加大量一级页面或把规划中的能力伪装为现有功能。
- 购票、行情、外部通知等属于后续受控工具：没有真实适配器、权限、数据来源和用户确认时，只能说明尚不可用，不能声称已查询、预订、购买或推送。
...
- 产品文案优先集中配置；关键业务注释解释“为什么”，不复述代码。删除资源前先确认引用，只清理确定无用且属于当前切片的内容。
...
- 默认把读者视为正在成长的新手：出现新概念时，必须用“它是什么 → 为什么这样做 → 在本项目哪里体现 → 带来什么好处或代价”的顺序讲清楚，不能只罗列术语。
- 每次至少讲清一个产品判断、一个全栈设计点；涉及 Agent 时优先解释模型与 Java Harness 各自负责什么、边界为何这样划分，以及失控或失败时系统如何兜底。
- 避免重复课程：每次选择与本次改动最相关的一个新知识点深入讲；已经讲过且没有变化的原则只引用对应文档或简短回顾，只有取舍变化时才重新展开。
- 详细格式遵循 `docs/ENGINEERING_STANDARDS.md` 的“每次开发后的成长交付”。

## 当前阶段

M0 和 M1 的聊天确认核心闭环已完成；Skill 管理前端仍有一项待补。当前主目标是 `docs/DEVELOPMENT_PLAN.md` 中的 M2：完成首个“真实来源 → Evidence → Claim → Policy Gate → 动态 → 学习内容”纵向闭环。不要并行铺开购票、行情、通知、复杂在线编辑器、社区、Skill 市场或微服务基础设施。
