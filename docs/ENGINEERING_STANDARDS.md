...
- 颜色、字体、间距和动效使用设计系统语义令牌；明暗主题不得靠页面级硬编码覆盖。
- Markdown 只作为存储/传输格式，展示时解析为安全 React 节点；禁止直接注入模型返回的 HTML。
- AI 生成正文与用户批注分开持久化。核验或 AI 内容通过反馈和受控重生成修改，不使用通用富文本编辑器破坏来源语义。
...
讲解默认控制在 5–10 个要点内：不堆术语，不逐文件复述，不省略风险；遇到新概念时按“它是什么 → 为什么这样做 → 在本项目哪里体现 → 带来什么好处或代价”说明。

每次选择一个与本次改动最相关、此前未深入讲解的知识点；已经讲过且设计没有变化的内容只引用本节或简短回顾，避免重复课程。涉及 Agent 时，优先用新手能理解的语言说明模型与 Java Harness 各自负责什么、边界为什么这样划分，以及失败或失控时系统如何兜底。
